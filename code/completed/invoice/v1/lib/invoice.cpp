...
namespace invoice::v1 {

void AddLineForSingleItem(
    const std::string& item, const ProductMap& products, int quantity, int& dairyItems,
    double& dairyTotal, int& produceItems, double& produceTotal,
...
std::string GenerateInvoice(const Order& order, const ProductMap& products) {
    ...
    for (const auto& [item, quantity] : order.second) {
        AddLineForSingleItem(
            item, products, quantity, dairyItems, dairyTotal, produceItems,
            produceTotal, result
        );
    }
