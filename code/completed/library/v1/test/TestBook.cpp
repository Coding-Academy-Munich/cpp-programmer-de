...
TEST_CASE("Test BookInfo")
{
    ...
    SECTION("Test operator=!()")
    {
        auto book {BookInfo::Create("isbn", "title", {"author1", "author2"},
        ...
        REQUIRE(*book != *bookIsbn);
        REQUIRE(*book != *bookAuthors);
        REQUIRE(*book != *bookPublisher);
        REQUIRE(*book != *bookPubYear);
        REQUIRE(*book != *bookEdition);
    }

    SECTION("Test operator<<()")
    {
        auto book {BookInfo::Create("1234", "My Book", {"Joe Random", "Jane Doe"},
