...
TEST_CASE("Library can be created", "[library]")
{
    ...
    SECTION("When books and members are provided")
    {
        ...
        Library::BookVector books;
        books.push_back(std::make_unique<Book>(cppBookInfo));
        books.push_back(std::make_unique<Book>(cppBookInfo));

        Library::MemberVector members;
        members.push_back(
            std::make_unique<Member>(2, std::make_unique<RegularMemberType>(),
                "Jane Doe", "123 Main Street", MakeYearMonthDay(1960, 2, 3)));
        members.push_back(
            std::make_unique<Member>(1, std::make_unique<RegularMemberType>(),
