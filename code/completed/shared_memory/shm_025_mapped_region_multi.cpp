...
#include "shm_025_mapped_region_multi.h"
#include "utils.h"
...
int main(int argc, char* argv[]) {
    if (argc == 1) {
        print_header("Shared Memory Example 025: Mapped Region Multi");
        return shm025::run_parent(argv[0]);
    } else if (argc == 2) {
        return shm025::run_child(argv[1]);
    } else {
...
constexpr std::size_t double_allocation_start {char_allocation_size};
constexpr std::size_t num_double_allocations {1024};
constexpr std::size_t double_allocation_size {num_double_allocations * sizeof(double)};
static_assert(
    char_allocation_size + double_allocation_size <= shared_memory_length_in_bytes
);
...
int run_parent(std::string_view program) {
    ...
    // Fill another part of the memory with the pattern 0.0, 1.0, 2.0, ...
    double* double_data {reinterpret_cast<double*>(char_data + double_allocation_start)};
...
int run_child(std::string_view arg) {
    ...
    // Access double_data in the shared memory region
    double* double_data {reinterpret_cast<double*>(char_data + double_allocation_start)};
